...
public function markConfirmedAsSent(Request $request)
    ...
    public function index()
    {
        ...
        $productos = Producto::all(['id', 'nombre']); // Obtiene tanto el id como el nombre
    ...
    public function getPedidoData(Request $request)
    {
        ...
        if ($request->id_pedido) {
            ...
            foreach ($pedidoProductos as $pedidoProducto) {
                $productos[] = $pedidoProducto->producto->nombre; // Concatenamos el nombre de los productos
                $cantidad_productos += $pedidoProducto->cantidad; // Sumamos la cantidad de productos
                $monto_deposito += $pedidoProducto->cantidad * $pedidoProducto->precio; // Sumamos el monto total
            }

            return response()->json([
                'productos' => implode(', ', $productos), // Concatena los productos separados por coma
                'cantidad_productos' => $cantidad_productos,
                'monto_deposito' => number_format($monto_deposito, 2) // Devuelve el monto total
            ]);
        }
